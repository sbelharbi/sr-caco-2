...
def _get_basics(path_ds: str, sizes: dict, path_pred: str, path_interp: str):

    # super res.
    files = _get_all_files_size(path_ds, sizes['1024'], 'tif')
    super_res = _files_to_dict(files)

    # pred
    files = find_files_pattern(path_pred,  f'*.png')
    pred = _files_exp_to_dict(files)

    # interpolated
    files = find_files_pattern(path_interp, f'*.png')
    interp = _files_exp_to_dict(files)

    for k in pred:
        assert k in interp, k
        assert k in super_res, k
...
def _plot_images(super_res: np.ndarray,
                 pred: np.ndarray,
                 inter: np.ndarray,
                 fout: str,
                 images: dict,
    ...
    _show_img_and_tag(axes[0, 0], images['super_res'], f'HR.{h}')
    _show_img_and_tag(axes[0, 1], images['pred'], f'PRED.{h}')
    _show_img_and_tag(axes[0, 2], images['interp'], f'INTER.{l}')
...
if __name__ == '__main__':
    ...
    # check min max.
    # min_max(path_ds, sizes, 'tif')

    # get all basics: gt, pred, interpolated.
    # swinir:
    exp_fd = '??'

    # DSR-SPLINES

    path_pred = join(root_dir, exp_fd, "??")
    path_interp = join(root_dir, exp_fd, "??")

    super_res, pred, interp = _get_basics(path_ds, sizes, path_pred=path_pred,
                                          path_interp=path_interp)


    # compare: super res, pred, interp
    compare(super_res, pred, interp, join(root_dir, exp_fd, 'comparison'),
            l=128, h=1024)
