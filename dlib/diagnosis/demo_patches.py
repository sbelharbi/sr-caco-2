...
def low_to_h_mixed(l_patches: torch.Tensor, dict_atoms_l_to_high: dict,
    ...
    for i in tqdm(range(l_patches.shape[0]), total=npatches, ncols=80,
                  disable=silent):
        hash_atom_l = ordered_keys_l[idx_patch_l[i]]
...
if __name__ == '__main__':
    ...
    master_dataset = constants.BIOSR

    if master_dataset == '??':  # fixit
        INSZ = 512
        OUTSZ = 1024
    ...
    parser.add_argument("--cell", type=str, default='1', help="cell name.")
    parser.add_argument("--sz_l", type=int, default='5',
    ...
    if COMPUTATION == _VECTORIZED:
        ...
        evaluate_mixed(
            test_paths, sz=str(INSZ), super_sz=super_sz, outdir=outd_tst, sz_l=sz_l,
