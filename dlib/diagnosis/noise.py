...
def process_scale(paths: dict, sz: str, super_sz: str, nbr_s_interm: int,
    ...
    for k in tqdm(paths[sz], ncols=80, total=n):
        ...
        images = []
        l_scales = []
        for i in np.arange(1, scale, scale/nbr_s_interm):

            if var_type == constants.VAR_GLOBAL:
                if i == 1:
                    pass
                else:
                    _h = int(h * i)
                    _w = int(w * i)
                    img = Image.fromarray(img).resize((_w, _h),
                                                      resample=PIL.Image.BICUBIC)
                    img = np.array(img)

                img = add_noise_global_var(img, var_global).astype(np.uint8)

            elif var_type == constants.VAR_ADAPTIVE:
                pass

            else:
                raise NotImplementedError(var_type)

            images.append(_build_img(img, cell_type))
            l_scales.append(i)

        img = Image.fromarray(img).resize((int(super_sz), int(super_sz)),
                                          resample=PIL.Image.BICUBIC)
        img = np.array(img)

        img = add_noise_global_var(img, var_global).astype(np.uint8)
        images.append(_build_img(img, cell_type))
        l_scales.append(scale)
...
if __name__ == '__main__':
    ...
    fdout = join(root_dir, 'data/debug/noise')
    os.makedirs(fdout, exist_ok=True)
