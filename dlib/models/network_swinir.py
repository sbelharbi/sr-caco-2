...
# Jingyun Liang, Jiezhang Cao, Guolei Sun, Kai Zhang, Luc Van Gool,
# Radu Timofte,
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True,
        ...
        relative_coords[:, :, 0] += self.window_size[0] - 1  # shift to start
        # from 0
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
        ...
        assert 0 <= self.shift_size < self.window_size, "shift_size must" \
                                                        " in 0-window_size"
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                drop_path=drop_path[i] if isinstance(drop_path, list) else
                drop_path,
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 resi_connection=constants.R_CONNECTION_1CONV):
        ...
        if resi_connection == constants.R_CONNECTION_1CONV:
            self.conv = nn.Conv2d(dim, dim, 3, 1, 1)
        elif resi_connection == constants.R_CONNECTION_3CONV:
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        x = x.transpose(
            1, 2).view(B, self.embed_dim, x_size[0], x_size[1])  # B Ph*Pw C
        return x
...
class SwinIR(nn.Module):
    ...
    def __init__(self,
        ...
        if self.upsampler == constants.US_PIXEL_SHUFFLE:
            # for classical SR
        ...
        elif self.upsampler == constants.US_PIXEL_SHUFFLE_DIRECT:
            # for lightweight SR (to save parameters)
            self.upsample = UpsampleOneStep(upscale, embed_dim, num_out_ch,
                                            (patches_resolution[0],
                                             patches_resolution[1]))
        elif self.upsampler == constants.US_NEAREST_CONV:
...
if __name__ == '__main__':
    ...
    s = 128
    in_chans = 1
    height = (s // upscale // window_size + 1) * window_size
    width = (s // upscale // window_size + 1) * window_size
    ...
    model = SwinIR(upscale=upscale,
                   ...
                   mlp_ratio=2, upsampler=constants.US_PIXEL_SHUFFLE_DIRECT)
    model.to(device)
    print(model)
    print(s, height, width, model.flops() / 1e9)

    # x = torch.randn((1, 3, height, width), device=device)
    x = torch.randn((16, in_chans, s // upscale, s // upscale), device=device)
    with torch.no_grad():
