...
def evaluate():
    ...
    use_interpolated_low = parsedargs.use_interpolated_low

    assert split in constants.SPLITS, split
    assert os.path.isdir(exp_path)

    _CODE_FUNCTION = f'inf-super-res-eval_ds_name-{eval_ds_name}'

    _VERBOSE = True
    with open(join(exp_path, 'config_final.yml'), 'r') as fy:
        args: dict = yaml.safe_load(fy)
        args['c_cudaid'] = str(cudaid)
        args['use_interpolated_low'] = use_interpolated_low  # todo: modified.
        args = Dict2Obj(args)
    ...
    save_img_dir = join(outd, 'images',
                        f"use_interpolated_low-{use_interpolated_low}",
                        split, eval_ds_name)
...
def build_cmds():
    ...
    for i in range(_NBRGPUS):
        _script_path = join(root_dir, 'super_res_c_{}.sh'.format(i))
        script = open(_script_path, 'w')
        script.write(PREAMBULE)
        for file in splitted_files[i]:
            exp_dir = dirname(file)

            cmd = f'python dlib/inference/inference_super_res.py ' \
                  f'--cudaid 0 ' \
