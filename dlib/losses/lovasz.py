...
from dlib.utils.constants import BINARY_MODE, MULTICLASS_MODE, MULTILABEL_MODE
...
def _lovasz_hinge(logits, labels, per_image=True, ignore=None):
    ...
    if per_image:
        loss = mean(
            _lovasz_hinge_flat(
                *_flatten_binary_scores(log.unsqueeze(0),
...
def _lovasz_softmax(probas, labels, classes="present", per_image=False,
    ...
    Args:
        @param probas: [B, C, H, W] Variable, class probabilities at each
        prediction (between 0 and 1).
        Interpreted as binary (sigmoid) output with outputs of size [B, H, W].
        @param labels: [B, H, W] Tensor, ground truth labels
        (between 0 and C - 1)
        @param classes: 'all' for all, 'present' for classes present in labels,
...
def _lovasz_softmax_flat(probas, labels, classes="present"):
    """Multi-class Lovasz-Softmax loss
    Args:
        @param probas: [P, C] Variable, class probabilities at each prediction
                       (between 0 and 1)
...
def _flatten_probas(probas, labels, ignore=None):
    ...
    C = probas.size(1)
    probas = torch.movedim(probas, 0, -1)  # [B, C, Di, Dj, Dk...] -> [B, C,
    # Di...Dk, C]
...
class LovaszLoss(_Loss):
    def __init__(
        ...
        Args:
            ...
            per_image: If True loss computed per each image and then averaged,
            else computed per whole batch
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.mode in {BINARY_MODE, MULTILABEL_MODE}:
            loss = _lovasz_hinge(y_pred, y_true, per_image=self.per_image,
                                 ignore=self.ignore_index)
        elif self.mode == MULTICLASS_MODE:
            y_pred = y_pred.softmax(dim=1)
            loss = _lovasz_softmax(y_pred, y_true, per_image=self.per_image,
                                   ignore=self.ignore_index)
        else:
